...
begin
   ...
   AC_stimuli[counter][4] = sub_String[5]; # timing condition
end;  
...
loop j = 1 until j > 3 begin # one block
	if j == 2 then # timing one: first A then C
		desc.set_caption(" ");
		desc.redraw();
		AClearn_trial.set_duration(1500);
		wordAC.set_caption(AC_prac[1][j]);
		wordAC.redraw();
		AClearn_trial.present();
		AClearn_trial.set_duration(2500);			
		desc.set_caption(AC_prac[2][j]);
		desc.redraw();
		AClearn_trial.present();			
	else # timing two: A and C together
...
loop i = 1 until i > 10	begin # 10 blocks
	...
	# AC-learning + questions
	loop j = 1 until j > 8 begin # one block
		if int(AC_stimuli[(i-1)*8+j][4]) == 1 then # timing one: first A then C
			desc.set_caption(" ");
			desc.redraw();
			AClearn_trial.set_duration(1500);
			...
			desc.redraw();
			AClearn_trial.present();			
		elseif int(AC_stimuli[(i-1)*8+j][4]) == 2 then # timing two: A and C together
			AClearn_trial.set_duration(4000);
			wordAC.set_caption(AC_stimuli[(i-1)*8+j][1]);
			wordAC.redraw();
			desc.set_caption(AC_stimuli[(i-1)*8+j][2]);
		...
		last_response = stimulus_manager.last_stimulus_data();
		ofile2.print(string(last_response.button()) + "\t" + string(last_response.reaction_time()) + "\t" + AC_stimuli[(i-1)*8+j][3] + "\t" + AC_stimuli[(i-1)*8+j][4] + "\n"); # button + RT + congruency condition + timing condition
