...
begin
   ...
   BC_rec_stimuli[counter][4] = sub_String[4]; # timing condition
...
ofile1.print("Description\tCongruency\tTiming\tRecognition\tConfidence\tRT\tPicture description\tRT\tPicture recognition\tConfidence\tRT\n");
counter_lures = 1; 
...
loop i = 1 until i > total	begin
	...
	last_response = stimulus_manager.last_stimulus_data();
	remaining_time = 4000 - last_response.reaction_time();
	ofile1.print(BC_rec_stimuli[i][1] + "\t" + BC_rec_stimuli[i][3] + "\t" + BC_rec_stimuli[i][4] + "\t");
