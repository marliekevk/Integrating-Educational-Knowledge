% Randomization of Educational schema inference, experiment 2 - MvK 2017
...
for s=30
    ...
    % make blocks  
    cntr = 0;
    for i = 1:8 % 8 blocks, every block gets 8 items, 2 per condition
    ...
    while ok == 0
        ...
        for i=1:total-3
            ...
            elseif stims_recall(i) < (total/2)+1 && stims_recall(i+1) < (total/2)+1 && stims_recall(i+2) < (total/2)+1 % three con in a row
                nr = 3;
                while nr ~= 0 
                    if stims_recall(i+nr) > total/2+1
                        temp = stims_recall(i+2);
    ...
    for row=1:total
        fprintf(fid_enc, '%s\t%s\t%i\t%s\n', cell2mat(stims_enc{row,1}),cell2mat(stims_enc{row,2}),stims_enc{row,3},cell2mat(stims_enc{row,4}));      
    end 
    ...
    for row=1:total*2
        fprintf(fid_rec, '%s\t%s\t%i\n', cell2mat(stims_rec{row,1}),cell2mat(stims_rec{row,2}),stims_rec{row,3});      
    end  
