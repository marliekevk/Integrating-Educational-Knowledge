...
%     inc_reac_miss = mean(nonzeros(inc_data(find(inc_data(:,3)==0),2)));
    ...
    % calculate metamemory as a measure of reactivation
    con_reac1_met = mean(con_data(find(con_data(:,2)==1),1));
    con_reac2_met = mean(con_data(find(con_data(:,2)==2),1)); 
    con_reac3_met = mean(con_data(find(con_data(:,2)==3),1)); 
    inc_reac1_met = mean(inc_data(find(inc_data(:,2)==1),1));  
    inc_reac2_met = mean(inc_data(find(inc_data(:,2)==2),1));
    inc_reac3_met = mean(inc_data(find(inc_data(:,2)==3),1));
    ...
    outcomes(s,:) = [con_av, con_cur2, con_cur3, con_reac1, con_reac2, con_reac3, con_cur2_tr, con_cur3_tr, con_reac1_tr, con_reac2_tr, con_reac3_tr, ...
        ...
        con_reac1_met, con_reac2_met, con_reac3_met, inc_reac1_met, inc_reac2_met, inc_reac3_met];
